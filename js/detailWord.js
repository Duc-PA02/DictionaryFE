...
const app = {
    ...
    displayData(data) {
        ...
        data.synonymsList.forEach(syn => {
            contentDiv.innerHTML += `<a href="#" onclick="app.fetchWord('${syn.synonym.name}')">${syn.synonym.name}</a>, `;
        });
        contentDiv.innerHTML += '</div>';

        contentDiv.innerHTML += '<div class="section"><h3>Antonyms:</h3>';
        data.antonymsList.forEach(ant => {
            contentDiv.innerHTML += `<a href="#" onclick="app.fetchWord('${ant.antonym.name}')">${ant.antonym.name}</a>, `;
        });
        contentDiv.innerHTML += '</div>';
    },
